...
and g' prev = function
    ...
    | Tail, (Asm.Set _ | Asm.SetL _ |  Asm.Mov _ | Asm.Neg _ | Asm.Add _ | Asm.Sub _ |Asm.Mul _ | Asm.Div _ | Asm.SLL _ | Asm.SRL _ | Asm.LW _ 
        as exp) -> let tmp = Id.gentmp Type.Int in
            Return(Value(tmp, Type.Int)) :: g' prev (NonTail(tmp), exp)
    ...
    Asm.FMul _ | Asm.FDiv _  | Asm.LWC _ as exp) -> let tmp = Id.gentmp Type.Float in
            Return(Value(tmp, Type.Float)) :: g' prev (NonTail(tmp), exp)
    ...
    | Tail, Asm.CallDir(l, ys, zs) -> Return(Dir(x, ys, zs)) :: prev
...
let print_inst = function
    ...
    | SetCLV(x, l) -> print_string ("SetCLV " ^ x ^ ", " ^ l ^ "\n")
    | Mov(x, y) -> print_string ("Mov " ^ x ^ ", " ^ y ^ "\n")
    | Neg(x) -> print_string ("Neg " ^ x ^ "\n")
    ...
    | CallDir(x, ys, zs, dest) -> print_string ("CallDir " ^ x ^ ", (" ^ string_of_id_list ys ^ "), (" string_of_id_list zs ^ "), dest: " ^ dest ^ "\n")
    | Save(x, y) -> print_string ("Save " ^ x ^ ", " ^ y ^ "\n")
    | Restore(x, y) -> print_string ("Restore " ^ x ^ ", " ^ y ^ "\n")
    | Return(Value(None)) -> print_string "Return None\n"
    | Return(Value(x, typ)) -> (match typ with
                                | Type.Int -> print_string "Return " ^ x ^ " : int\n"
                                | Type.Float -> print_string "Return " ^ x ^ " : float\n"
                                | _ -> print_string "Return " ^ x ^ "\n") 
    | Return(Cls(x, ys, zs)) -> print_string ("Return Cls(" ^ x ", (" ^ string_of_id_list ys ^ "), (" ^ string_of_id_list zs ^ "))")
    | Return(Dir(x, ys, zs)) -> print_string ("Return Dir(" ^ x ", (" ^ string_of_id_list ys ^ "), (" ^ string_of_id_list zs ^ "))")
