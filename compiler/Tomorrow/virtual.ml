...
let rec g env = function (* generate virtual machine code *)
    ...
    | Closure.Float(d) -> 
        ...
        with Not_found ->
            ...
            l in
            let x = Id.genid "l" in
            Let((x, Type.Int), SetL(l), Ans(LWC(x, 0)))
    ...
    | Closure.MakeCls((x, t), {Closure.entry = l; Closure.actual_fv = ys}, e2) ->
        ...
        let offset, store_fv = 
            expand
                ...
                (4, e2')
                (fun y offset store_fv -> seq(SWC(y, x, offset), store_fv))
    ...
    | Closure.Tuple(xs) ->
        ...
        expand
            ...
            (0, Ans(Mov(y)))
            (fun x offset store -> seq(SWC(x, y, offset), store))
    ...
    Closure.body = e} =
        ...
        let (offset, load) =
            expand
                ...
                (fun z offset load -> fletd(z, LWC(reg_cl, offset), load))
