...
$tests = array( "print", "sum-tail", "gcd", "sum", "fib", "ack", "even-odd", "adder", "funcomp", "cls-rec", "cls-bug", "cls-bug2", "shuffle", "spill", "spill2", "spill3", "join-stack", "join-stack2", "join-stack3", "join-reg", "join-reg2", "non-tail-if", "non-tail-if2", "inprod", "inprod-rec", "inprod-loop", "matmul", "matmul-flat", "fdiv", "float", "mandelbrot", "harmonic");
...
foreach ( $tests as $t ) {
	...
	$remain--;
	system("cat {$testdir}{$t}.s {$gitroot}compiler/Tomorrow/libmincaml.S | {$gitroot}asm/asm - {$testdir}{$t}.bin", $ret);
	if ( $ret != 0 ) {
		fail($remain);
		continue;
	}
